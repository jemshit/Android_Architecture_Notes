...
concerns in presentation logic*" [[1]](#references).
...
that we can test it without using instrumentation tests" [[3]](#references).

- "...One common attribute of MVP is that there has to be a **lot of two-way dispatching.** For example, when someone clicks the "Save" button, the event handler delegates to the Presenter's "OnSave" method. Once the save is completed, the Presenter will then call back the View through its interface so that the View can display that the save has completed." [[19]](#references)
...
getting the data from the disk, memory, or network" [[9]](#references).
...
Transformation is done asynchronously in Presenter [[5]](#references). Other use cases can be some annotations on fields, model must implement or extend ORM class/Android class (e.g. Parcelable), fields for (UI) state... which some other layers do not care about [[16]](#references).

## 2. Presenter

- "Presenter is a layer that provides View with data from Model.
Presenter also handles background tasks" [[6]](#references). It is **Logic of UI** [[2]](#references).

- "The Presenter is the 'middle-man' (played by the controller
in MVC) and has references to both, view and model" [[5]](#references). "**It retrieves data from the model and returns it *formatted* to the view**. *Decides the actions to take when input events are received*" [[10]](#references).

- **No Android code here** [[8]](#references), it will make platform independent and no instrumentation test is required. "As far as tests are concerned, most of the code that you absolutely need to test will be in the Presenter. What's great is that all this code doesn't need Android to run, as it just has a reference to the View interface, and not its Android-specific implementation. This means that you can just mock the View interface, and write pure JUnit tests that make sure the right methods are called on that mock to test the *integrity* of your business logic" [[10]](#references).
...
the Presenter needs to take specific actions" [[10]](#references) [[8]](#references). "Presenters do not need lifecycle callbacks from Activity, Fragment. It just needs to know if View is attached or not" [[11]](#references).

- "**Android Services** are a fundamental part of android app development... Therefore it's obvious that the Presenter is responsible to communicate with the Service" [[12]](#references).
...
the problem". You should cache in Model layer [[9]](#references) [[13]](#references). But this way you won't save ongoing network requests. **Presenter is only for UI logic, not data logic (cache).**

## 3. View

- **Display of UI** [[2]](#references). "View is a layer that *displays data* and *reacts to user actions*" [[6]](#references) [[5]](#references). "On Android, this could be an Activity, a Fragment, an android.view.View or a Dialog" [[6]](#references) [[7]](#references). "*The View doesn't know anything about the business logic or how to get the data* it has to show to the user... It will not contain any functional or business logic like access to database or web server for example" [[14]](#references). "View layer with MVP becomes so simple, so it does not even need to have callbacks when requesting for data" [[6]](#references). **View never calls Model directly**.
...
to other screens [[10]](#references).

- "*The view should concern only about any necessary request parameters
to restore the state*" [[8]](#references). It shouldn't save presenter or data itself.
...
multithreading. (returning data from Async calls)" [[5]](#references).
...
send them to the presenter" [[8]](#references).

- "You can also move all the way to having the presenter do all the manipulation of the widgets. This style, which I call **Passive View** isn't part of the original descriptions of MVP but got developed as people explored *testability issues*. I'm going to talk about that style later, but that style is one of the flavors of MVP.” [[17]](#references)
...
Figure reference: [[18]](#references)

#### b) Supervising Controller

- "The direction behind Bower and McGlashan was what I'm calling **Supervising Controller**, where the view handles a good deal of the view logic that can be described **declaratively** and the presenter then comes in to handle more **complex cases.**" [[17]](#references)
...
- "*Since it's an interface you can change the view implementation*.
    ...
    to something that extends Fragment." [[5]](#references)
...
- "Modularity: You can move the whole business logic, Presenter and
    ...
    this library with the containing Presenter in various apps." [[5]](#references)
...
- "*You can easily write unit tests since you can mock views by
    ...
    Presenter objects even more easy." [[5]](#references)
...
- "Another very nice side effect of defining an interface for the
    ...
    especially if you work in a team." [[5]](#references)
...
- "The methods to update the View (in View Interface) should be simple
    ...
    **formatting what should be displayed should be the responsibility of the Presenter.** For example, you might want to start displaying 'You'
    ...
    message, and this is part of your business logic" [[10]](#references).


##### MVP soundbites:

- "User gestures are handed off by the widgets to a Supervising Controller. (a presenter)" [[17]](#references)

- "The presenter coordinates changes in a domain model." [[17]](#references)

- "Different variants of MVP handle view updates differently. These vary from using. Observer Synchronization to having the presenter doing all the updates with a lot of ground in-between." [[17]](#references)
...
- Using Singleton PresenterManager class and retaining Presenters. Presenters store loaded models and returns them after orientation change [[10]](#references) [[6]](#references)

- Using LoaderManager to retain Presenter. This way ongoing network requests do not get lost [[15]](#references)

- Retain Presenter until View totally gets destroyed

- Caching API response Observables in LRUCache [[9]](#references)

- Storing API request Observables in somewhere (not to make requests twice)

- Caching API response data in cache, database, file...

- Request parameters for Presenter are saved in View (Activity...) [[6]](#references)
...
the application is in the background." [[6]](#references)
...
- restart background requests in case of a process restart" [[6]](#references)
...
- The Presenter should be recreated after process recreation (if process of our app was killed because of low memory issues) -- we must ensure reliability and possibility to restore presenter state later even if process is destroyed" [[15]](#references)
